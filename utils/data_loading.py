...
root_path = '/PROYECTO-VIT/data/'

def redimensionamiento_imagenes(root_path):
    """Método que se utiliza para redimensionar las imágenes
    antes del entrenamiento para disminuir el tiempo de ejecución.
    """
    for path_file in [root_path + 'yoga_train.txt', root_path + 'yoga_test.txt']:
        with open(path_file, 'r') as f:
            for line in f:
                img, label_6, label_20, label_82 = line.split(',')
                try:
                    image_path = os.path.join(root_path + 'Images', img)
                    img_ = Image.open(image_path)
                    # print(image_path)
                    img_ = img_.resize((224,224))
                    img_ = img_.convert('RGB') # RGBA --> RGB
                    img_.verify()
                    img_.save(image_path)
                    img_.close()
                    # img.verify()
                except:
                    pass
...
class Yoga_82(Dataset):
    ...
    def __init__(self, root, file_, transform = None, prueba = True, nivel = 6, n_samples = None):
        # atributos de clase
        self.root = root
        self.file_ = file_
        self.nivel = nivel
